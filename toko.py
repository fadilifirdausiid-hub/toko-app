...
if menu == "Dashboard":
    ...
    if not df_produk.empty:
        ...
        c1, c2 = st.columns(2)
        c1.metric("📥 Stok Masuk", f"Rp {int(total_masuk):,}")
        c2.metric("📦 Sisa Stok", f"Rp {int(total_sisa):,}")

        st.markdown("---")

        c3, c4 = st.columns(2)
        c3.metric("💵 Cash", f"Rp {int(saldo_cash):,}")
        c4.metric("🏦 Bank", f"Rp {int(saldo_bank):,}")
...
# ================= OWNER ORDER =================
    ...
    if not df_t.empty:
        ...
        for i, row in owners.iterrows():
            ...
            if col[3].button("Bayar", key=f"b{i}"):
...
# ================= CLOSING =================
    ...
    cash = df_bayar[df_bayar["metode"]=="cash"]["jumlah"].sum() if not df_bayar.empty else 0
    bank = df_bayar[df_bayar["metode"]=="bank"]["jumlah"].sum() if not df_bayar.empty else 0
